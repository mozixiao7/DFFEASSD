...
def compute_eer_and_tdcf(cm_score_file, path_to_database):
    asv_score_file = '.\scores\ASVspoof2019.LA.asv.eval.gi.trl.scores.txt'
    ...
    ax = plt.subplot(121)
    plt.hist(tar_asv, histtype='step', density=True, bins=50, label='目标说话人')
    plt.hist(non_asv, histtype='step', density=True, bins=50, label='非目标说话人')
    plt.hist(spoof_asv, histtype='step', density=True, bins=50, label='伪造语音')
    plt.plot(asv_threshold, 0, 'o', markersize=10, mfc='none', mew=2, clip_on=False, label='EER 阈值')
    ...
    ax = plt.subplot(122)
    plt.hist(bona_cm, histtype='step', density=True, bins=50, label='真实语音')
    plt.hist(spoof_cm, histtype='step', density=True, bins=50, label='伪造语音')
